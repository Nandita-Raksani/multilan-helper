...
/**
 * Get translation subdirectories (EB, PCB, Business, etc.)
 * Skips files and the 'converted' directory.
 */
function getTraSubdirs(traDir) {
  return fs.readdirSync(traDir).filter(entry => {
    if (entry === 'converted') return false;
    const fullPath = path.join(traDir, entry);
...
function convertTraFiles() {
  ...
  const subdirs = getTraSubdirs(traDir);

  console.log('Converting .tra files to UTF-8...');
  console.log(`Found translation folders: ${subdirs.join(', ') || '(none)'}`);

  for (const file of traFiles) {
    const parts = [];
    for (const subdir of subdirs) {
      const inputPath = path.join(traDir, subdir, file);
      if (fs.existsSync(inputPath)) {
        console.log(`  ${subdir}/${file}:`);
        parts.push(readFileAsUtf8(inputPath));
      }
    }
    ...
    if (parts.length > 0) {
      ...
      console.log(`  ${file}: NOT FOUND in any folder - skipping`);
    }
  }
...
function generateTraModule() {
  ...
  console.log('Generating tra-bundle.ts (compressed, folder-keyed)...');
  console.log(`Found translation folders: ${subdirs.join(', ') || '(none)'}`);
  ...
  moduleContent += '// Translation data is compressed with deflate and stored as base64.\n';
  moduleContent += '// Each folder\'s data is decompressed lazily when first accessed.\n';
  moduleContent += '// This reduces plugin bundle size by ~8-10x for large .tra files.\n\n';
  moduleContent += 'export interface CompressedFolder {\n';
  moduleContent += '  en: string;\n';
  moduleContent += '  fr: string;\n';
  moduleContent += '  nl: string;\n';
  moduleContent += '  de: string;\n';
  moduleContent += '}\n\n';
  moduleContent += 'export const traFileContentsCompressed: Record<string, CompressedFolder> = {\n';

  for (const subdir of subdirs) {
    moduleContent += `  ${JSON.stringify(subdir)}: {\n`;
    ...
    for (const [lang, file] of Object.entries(traFiles)) {
      ...
      if (fs.existsSync(inputPath)) {
        ...
        const compressed = zlib.deflateRawSync(Buffer.from(content, 'utf8'), { level: 9 });
        const b64 = compressed.toString('base64');
        totalRaw += raw;
        totalCompressed += b64.length;
        const ratio = ((1 - b64.length / raw) * 100).toFixed(1);
        console.log(`  ${subdir}/${file}: ${(raw / 1024).toFixed(1)}KB → ${(b64.length / 1024).toFixed(1)}KB (${ratio}% reduction)`);
        moduleContent += `    ${lang}: "${b64}",\n`;
      } else {
        console.log(`  ${subdir}/${file}: NOT FOUND - using empty string`);
        moduleContent += `    ${lang}: "",\n`;
      }
    }
  ...
  moduleContent += '};\n\n';
  moduleContent += 'export const FOLDER_NAMES = ' + JSON.stringify(subdirs) + ';\n';

  fs.writeFileSync(outputPath, moduleContent, 'utf8');
  const overallRatio = ((1 - totalCompressed / totalRaw) * 100).toFixed(1);
  console.log(`\nTotal: ${(totalRaw / 1024).toFixed(1)}KB → ${(totalCompressed / 1024).toFixed(1)}KB (${overallRatio}% reduction)`);
